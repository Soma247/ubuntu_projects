...
;fill zeroes
   ...
   mov rcx, length
str_loop2:
   stosb; mov byte[rdi], al; if(!DF) inc rdi, else dec rdi
   loop str_loop2
   prnt my_string, length
;fill ones
   prnt string3, 19
   ...
   mov rdi, my_string
   mov rcx, length
   rep stosb;call stosb rcx times 
            ;rcx decrementing BEFORE ops!!!: while(--rcx; !ZF)stosb;
   prnt my_string, length
;fill ascii again
   prnt string4, 26
   mov rax, 32
...
;reverse_copy
   ...
   mov rcx, length
   rep stosb
   lea rsi, [my_string+length-4];only alphabeth
   lea rdi, [other_string+length]
   mov rcx, 27; copy only 27-1 symbols
   std; up DF flag, cld to clear DF
   rep movsb; while(rcx)
   ;movsb - copy [rsi]->[rdi],if(!DF)rdi++, rci++; else rdi--, rci--;
   prnt other_string, length
   leave
   ret
